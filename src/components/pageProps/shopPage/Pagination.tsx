...
function Items({ currentItems, selectedBrands, selectedCategories }: any) {
  // Filter items based on selected brands and categories
  const filteredItems = currentItems.filter((item: any) => {
    const isBrandSelected =
      selectedBrands.length === 0 ||
      selectedBrands.some((brand: any) => brand.title === item.brand);

    const isCategorySelected =
      selectedCategories.length === 0 ||
      selectedCategories.some((category: any) => category.title === item.cat);
...
const Pagination = ({ itemsPerPage }: any) => {
  ...
  const selectedBrands = useSelector(
    (state: any) => state.orebiReducer.checkedBrands,
  );
  const selectedCategories = useSelector(
    (state: any) => state.orebiReducer.checkedCategorys,
  );
