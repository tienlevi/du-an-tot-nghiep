...
const SizeVariantItem = ({ item, updateQueryParam }: SizeVariantItemProps) => {
    ...
    return (
        <button
            ...
            className={`w-10 h-10 block text-sm text-opacity-90 transition-colors duration-700 cursor-pointer text-[#777777]  ${query?.size === item._id ? 'border-[#333333]' : 'border-[#d3d3d3] hover:border-[#333333]'} border-[1.5px] rounded-sm `}
        >
            {item.name}
        </button>
    );
